...
function New-ZXTokenSession{
    ...
    if($Save){
        ...
        $LogonData = @()
        $FilteredSessions = Get-Content -Raw $SaveLocation | ConvertFrom-Json
        $LogonData += $FilteredSessions | Where-Object {$_.URL.trim() -ne $NewObj.URL.trim() }
...
function Update-ZXHostTemplateList{
    ...
    if($UnlinkClearTemplateID){
        $ZXHost = Get-ZXHost -HostID $HostId -IncludeParentTemplates
        $TemplatesToClear = $ZXHost.ParentTemplates | Where-Object {$_.templateid -eq $UnlinkClearTemplateID} | Select-Object templateid
        $PSObj.params |  Add-Member -MemberType NoteProperty -Name "host" -Value $ZXHost.host
